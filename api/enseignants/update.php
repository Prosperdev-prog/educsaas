...
try {
    $stmt = $conn->prepare("UPDATE users SET nom = ?, prenom = ?, email = ? WHERE id = ? AND school_id = ? AND role = 'enseignant'");
    if ($stmt->execute([$nom, $prenom, $email, $id, $school_id])) {
        echo json_encode(["status" => "success", "message" => "Enseignant mis à jour avec succès"]);
    } else {
